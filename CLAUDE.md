...
- **Standing prohibitions:** new runtime deps (sole sanctioned exception: Radix Popover/Dialog, D19) · WebGL/3D in UI (D3) · motion outside `docs/08`'s sanctioned set — hero autoplay, Ken Burns, and showcase carousels ARE sanctioned per Amendment A (2026-07-11); everything else isn't · hardcoded dataset counts · fabricated statistics (D12) · dynamic Tailwind classes for data-driven colors · hand-editing generated artifacts (`lib/india-geo.ts`, `data/popularity.json`).
