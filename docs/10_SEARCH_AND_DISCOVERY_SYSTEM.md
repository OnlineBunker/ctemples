...
~15 canonical queries asserting top-3 membership/order, run in CI, updated only via file-14 amendment: `mahadev` → Shiva temples, alias fires · `balaji` → Tirumala #1 · `jagannath` → Puri #1 · `madurai` → Meenakshi #1 · `varanasi` → Kashi Vishwanath #1 · `golden temple` → Amritsar #1 · `jyotirlinga` → only Jyotirlinga-tagged, rating-ordered · `unesco` (tag-contains match — no alias needed, since a tag string like "UNESCO World Heritage" already contains the query) → the three UNESCO records · `meenakshee` (typo) → Meenakshi top-3 [activates with Meili; in-memory engine exempt] · `tamil nadu` → TN temples · `sun temple` → Konark #1 · `shiva` → Shiva set, rating-ordered · `himalayan` → Kedarnath present · `kanchipuram` → place-alias behavior (no dataset match today → zero results until such a temple exists; the alias may still ship) · empty → rating order. Engine swaps must pass this suite unchanged.
