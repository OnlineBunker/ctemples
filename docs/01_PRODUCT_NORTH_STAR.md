...
**Map & components**
- **D16. India GeoJSON source = datameet/maps** (CC-BY 2.5 IN; Survey-of-India-aligned boundaries — the legally required depiction for an India-market product; Natural Earth rejected for its de-facto boundary lines). Build pipeline: mapshaper simplify (visvalingam, ~4–5%, keep-shapes) → project once → quantize to a ~1000×1100 viewBox → generated `lib/india-geo.ts`; budget ≤80 KB raw. Visible attribution required. → file 07 §2.
