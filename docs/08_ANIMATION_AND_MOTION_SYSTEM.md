...
| 7 | Hero drifting birds | two `position:absolute` SVG gull strokes in the sky, `translate` drift ≈+80vw L→R with a small vertical bob, `linear infinite`, ~34s and ~46s (8s offset), low opacity, behind the arch/headline | not rendered / no animation — no birds |
