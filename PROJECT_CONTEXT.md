...
## Build status — as of 2026-07-14 (Phase 9)
...
| P10–P11 | ⬜ Not started | **Next: P10 — Perf & a11y hardening** per `docs/13`. |
...
**Proceed to Phase 10 — Perf & a11y hardening** per `docs/13_IMPLEMENTATION_MASTER_PLAN.md`. Start sequence for a future session:

1. Read this **Build status** section, then `docs/13`'s P10 row (Lighthouse ≥90/85 + axe-clean gates on all three page types, reduced-motion full audit against docs/08 §7's matrix, 200% zoom + screen-reader walkthroughs, image/JS budget verification per docs/04 §9/docs/05 §9).
2. Run Lighthouse + axe against `/`, `/explore` (both modes), and a `/temples/[id]` page; fix whatever fails the ≥90/85 floor.
3. Full reduced-motion audit against the docs/08 §7 matrix (every sanctioned animation has a documented reduced-motion behavior — verify each one, not just spot-check).
4. 200% browser zoom + a full screen-reader pass (not just spot-checks) across the three page types.
5. Finish on `typecheck && test && lint && build` green, browser verification, and an adversarial review pass, per `docs/14_CLAUDE_CODE_OPERATING_SYSTEM.md`.

**Two open items carried forward, neither blocking P10:**
- The D17 rest-map palette gives ~1.19:1 boundary contrast between adjacent unselected states (`#F4EADF` fill + `#FFFFFF` stroke), below WCAG 1.4.11's 3:1 for meaningful graphical boundaries (carried from P4). It is exactly what D17 specifies, so changing it is a D17 amendment — decide whether to darken the rest stroke. This is itself exactly the kind of contrast issue P10's own Lighthouse/axe gate should catch and force a decision on.
- The state-tile popover's theoretical first-open positioning question above (P9 adversarial review) — low-confidence, unconfirmed, worth a real-device spot-check if it's ever actually reported as visible.
