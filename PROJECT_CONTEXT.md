...
| P4–P11 | ⬜ Not started | **Next: P4 — Explore map mode** per `docs/13` §2. Needs a real, licensed GeoJSON source picked for `lib/india-geo.ts` (D16, §14.3) before building against it. `/temples/[id]` is still the pre-existing page (untouched, out of scope until P5). |
...
**Proceed to Phase 4 — Explore map mode** per `docs/13_IMPLEMENTATION_MASTER_PLAN.md` §2. Start sequence for a future session:

1. Read this **Build status** section, then `docs/07_INDIA_MAP_SYSTEM.md` in full, `docs/05_EXPLORE_EXPERIENCE.md` §6 (map mode UI), and `docs/03_WORLD_CLASS_DESIGN_SYSTEM.md` §6.8 (bottom sheet).
2. Pick a real, licensed GeoJSON source for `lib/india-geo.ts` first (D16: datameet/maps, CC-BY 2.5 IN) — this was deliberately deferred from Phase 0 and blocks everything else in this phase; do not fabricate boundary data.
3. Build the geometry pipeline, `IndiaMap` + region pills + clusters + callout markers + the a11y contract, the boundary-trace micro-interaction, the mobile bottom sheet, `checkCoordinateInState`, state-silhouette tiles, and the attribution caption + `/about` credit.
4. Finish on `typecheck && test && lint && build` green, browser verification (including the mobile bottom sheet's drag/keyboard snap points), and an adversarial review pass, per `docs/14_CLAUDE_CODE_OPERATING_SYSTEM.md`.
