...
| P3.5–P11 | ⬜ Not started | **Next: P3.5 — Content foundations** per `docs/13` §2 (tag registry, slug registry, popularity model, anti-slop lint, interim image attribution). `/temples/[id]` is still the pre-existing page (untouched, out of scope until P5). |
...
**Proceed to Phase 3.5 — Content foundations** per `docs/13_IMPLEMENTATION_MASTER_PLAN.md` §2. Start sequence for a future session:

1. Read this **Build status** section, then `docs/09_CONTENT_MODEL_20000_TEMPLES.md` (tag registry, popularity model, anti-slop rules) and `docs/11_BACKEND_EVOLUTION_PLAN.md` §8.3–7 (image attribution).
2. Build the tag registry + validator wiring, `lib/slug.ts` + slug registry, `compute-popularity.ts` + `data/popularity.json` + the `filter.ts` sort switch, anti-slop lint over the 15 records, and the interim image-attribution credit.
3. Finish on `typecheck && test && lint && build` green, browser verification, and an adversarial review pass (as done for Phases 2 and 3), per `docs/14_CLAUDE_CODE_OPERATING_SYSTEM.md`.
4. When Phase 4 (map mode) comes up, pick a real, licensed GeoJSON source for `lib/india-geo.ts` before building against it — see `docs/07_INDIA_MAP_SYSTEM.md` §2.
